...
computeTfByPos7( std::vector<double> _pos_fr, std::vector<double> _pos_to )
{
    ...
    /// 这里的pos是相机在世界坐标的下的位姿. 四元数变成旋转矩阵前，角度和平移要取反
...
template<class Pointxyz> std::vector<cv::DMatch>
getMatchsByTransform(const std::vector<Pointxyz> &_from, const std::vector<Pointxyz> &_to, const Eigen::Matrix4f &_trans, const bool &_cross_check, const double &_dis_thresh  )
{
    ...
    for(int i=0; i<_from.size(); i++ )
    {
        ...
        if( min_id_to != -1 && min_dist < _dis_thresh)//mm
...
template<class Pointxyz> cv::Mat
...
template<class Pointxyz> Eigen::Matrix4f
...
getMatchsByhomography( const std::vector<cv::KeyPoint> &_from, const std::vector<cv::KeyPoint> &_to, const cv::Mat& homography, const bool _cross_check )
{
...
std::vector<cv::DMatch> refineMatchesWithHomography( const std::vector<cv::KeyPoint>& _src, const std::vector<cv::KeyPoint>& _dst,
